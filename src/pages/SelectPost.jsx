...
const SelectPost = () => {// Starting time in seconds
  ...
  const [addComment, setaddComment] = useState(false); // Starting time in seconds
  const [commentContent, setcommentContent] = useState(""); // Starting time in seconds
  ...
  let goNext = () => {
    ...
    if (checkoutData.selected === "comments") {
      console.log("yes");
      setaddComment(true)
      return
    } else {
  ...
  let goNextPage = () => {
    if (commentContent.trim() === "") {
      toast.error('Escreva um comentário para prosseguir')
      return
    }
    updateCheckoutData("commentContent", commentContent)
    navigate("/pay/form")
    ...
    <>
      <div className="min-h-screen w-screen overflow-hidden relative">
        ...
        {checkoutData.username !== "" && checkoutData.userInfo?.length > 0 ?
          addComment ? <div className="lg:mt-12 mt-10">
